...
def negamax_ab(node, depth, columns_map, alpha=-float('inf'), beta=float('inf')):
    ...
    childlist = [child for child in node.create_children(columns_map=columns_map)]
    values = [child.value for child in childlist]
    plays = [child.play for child in childlist]
